...
void main() {
  ...
  group('ProductRemoteDataSource (Dio + Isolate Integration)', () {
    // Note: Dio natively decodes JSON strings into Maps/Lists automatically.
    // So our mock response will return a pre-parsed Map, exactly like real Dio does!
    final tMockDioResponseData = {
      {
        "products": [
          {
            "id": 1,
    ...
    test(
      '✅ Should return List<ProductModel> when Dio fetches data with status 200',
      () async {
        // ARRANGE: Intercept the dio.get call and return a type-safe Response container
        when(
          () => mockDio.get(any(), options: any(named: 'options')),
        ).thenAnswer(
          (_) async => Response(
            data: tMockDioResponseData,
            statusCode: 200,
            requestOptions: RequestOptions(path: ''),
          ),
        );

        // ACT: Execute your repository data parsing flow
        final result = await dataSource.getProducts();

        // ASSERT: Verify your Isolate processing converted the raw map into structural models
        expect(result, isA<List<ProductModel>>());
        expect(result.length, 1);
        expect(result.first.title, equals('Essence Mascara Lash Princess'));

        // VERIFY: Confirm that dio.get was invoked precisely once
        verify(
          () => mockDio.get(any(), options: any(named: 'options')),
        ).called(1);
      },
    ...
    test(
      '❌ Should throw a ServerException when Dio encounters a 404 error code',
      () async {
        // ARRANGE: Simulate a server error side by returning a 404 Response package
        when(
          () => mockDio.get(any(), options: any(named: 'options')),
        ).thenAnswer(
          (_) async => Response(
